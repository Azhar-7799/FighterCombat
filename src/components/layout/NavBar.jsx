    ...
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all ${shrink ? 'backdrop-blur glass border-b border-white/10 py-3' : 'py-4'}`}>
      <div className="max-w-7xl mx-auto flex items-center justify-between px-6">
        <div className="flex items-center gap-4">
          <img src={logo} alt="Fighters Combat Academy logo" className="h-12 w-12 rounded-3xl object-contain" />
          <div>
        ...
        <nav className="hidden items-center gap-2 xl:flex">
          {links.map(([label,to])=> (
            <NavLink key={to} to={to} end className={({isActive})=>`px-2 py-2 text-sm transition-all ${isActive ? 'text-yellow-400' : 'text-gray-300 hover:text-yellow-400'}`}>
              {label}
            </NavLink>
          ))}
          <NavLink to="/join" className="ml-2 rounded-full bg-yellow-500 px-4 py-2 text-sm font-semibold text-black shadow-lg shadow-yellow-500/20">Join</NavLink>
        </nav>
      ...
      {open && (
        <div className="border-t border-white/10 bg-black/95 px-6 py-4 backdrop-blur xl:hidden">
